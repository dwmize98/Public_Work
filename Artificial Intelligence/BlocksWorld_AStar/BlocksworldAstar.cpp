...
void astar (vector<string> initial_blocks, vector<string> final_blocks) {
    State* initial_state = new State(initial_blocks); 
    State* final_state = new State(final_blocks);
    Node* initial_node = new Node(nullptr, initial_state, 0, final_state);
    ...
    priority_queue<Node*, vector<Node*>, CompareWeight> frontier;
    ...
    while(!frontier.empty() && iter < 100000) {
        ...
        initial_node = frontier.top();
        frontier.pop();
        children_nodes = initial_node->successors();
        for (int i = 0; i < children_nodes.size(); i++) {
            child_node = children_nodes.at(i);
            if (child_node->goal_test(final_state)) {
                cout << "Success! iter=" << iter << ", cost=" << child_node->cost << ", depth=" << child_node->depth << ", max queue size=" << max << endl;
                child_node->print_path();
                return;
            }
            
            try {
                if (child_node->cost < reached.at(child_node->hash())->cost) {
...
int main (int argc, char** argv) {
    ...
    astar(initial_blocks, final_blocks);
